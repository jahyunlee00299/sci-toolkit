...
# ── 축 2: 미공개 연구 수치 ───────────────────────────────────────────────
# 260706 실측 — 미공개 MPSP($137.42/$84.09)와 수율(88.3%)이 스킬 파일에
...
# 금액·백분율·맨숫자는 그 자체로는 흔하다. 미공개 연구값으로 보는 것은 연구
# 문맥어가 함께 있을 때뿐이다 — 이 조합이 260706 에 실제로 유출된 형태다.
#
# 🔴 판별하는 것은 **숫자의 모양이 아니라 숫자 뒤에 오는 말**이다.
# 260807 실측으로 세 안을 비교했다(케이스 24건):
#   · 맨 소수만        → 오탐 1, 그러나 **유출 4/7 통과**(`수율이 92 였습니다`)
#   · 맨 정수까지 전부  → 유출 0 통과, 그러나 **오탐 13/13 = 정상 신고 전멸**
#   · 정수 + 수량어 제외 → 유출 0 통과, 오탐 0        ← 채택
# "92 였습니다"는 값이고 "3번째 줄"·"2개"·"5분"은 수량이다. 그 구분은
# 자릿수나 소수점이 아니라 뒤따르는 조사·단위어에 있다.
MONEY_PCT_RE = re.compile(
    r"(?:\$\s*\d+(?:\.\d+)?"      # 금액
    r"|\d+(?:\.\d+)?\s*%)"        # 백분율
)

# 문맥어가 있을 때 검사할 맨숫자(정수 포함).
BARE_NUMBER_RE = re.compile(r"(?<![\d.])\d+(?:\.\d+)?(?![\d.])")

# 숫자 **뒤**에 이런 말이 오면 수량·순서·시간이지 연구 측정값이 아니다.
# 이 목록이 곧 오탐 방어선이므로, 새 오탐이 보고되면 여기에 추가한다.
#
# 🔴 단위어 뒤에는 **경계가 필요하되 조사는 허용**해야 한다. 두 번 다 실측으로
# 배웠다(260807):
#   · 경계가 없으면 → `달`(개월)이 `달러`의 앞부분에 매치되어
#     `MPSP 120 달러` 가 통째로 통과한다(미탐).
#   · 경계를 `(?![가-힣])` 로 두면 → `2개로`·`3행에서`·`7개가` 처럼 조사가
#     붙은 정상 형태가 면제되지 못해 오탐 10건이 터진다.
# 그래서 "뒤에 오는 한글이 조사이거나, 한글이 아니면" 통과로 본다.
# 실명 탐지에서 겪은 조사 문제와 같은 뿌리다.
_COUNTERS = (
...
COUNTER_WORD_RE = re.compile(
    rf"^\s*(?:{_COUNTERS})"
    # 조사에 더해 서술 종결형(`3번째입니다`)도 허용한다. 조사만 넣으면
    # `번째입니다` 가 경계에 걸려 다시 오탐이 된다 — 실측 1건.
    r"(?:입니다|이다|이고|이며|째)?"
    r"(?:이|가|은|는|을|를|로|으로|에|에서|의|만|도|과|와|랑)?"
    r"(?![가-힣])"
    r"|^\s*(?:MB|GB|KB|TB|bytes?|byte|px|dpi)\b")

# 숫자 **앞**이 이런 말이면 값이 아니다.
#   · 버전·단축키: "Python 3.11", "Ctrl+7"
#   · 식별자: "이슈 #42", "PR #7" — `#` 뒤 숫자는 번호이지 측정값이 아니다
PRE_TECHNICAL_RE = re.compile(
    r"(?:Python|Ctrl|Alt|Shift|v|version|버전)\s*\+?\s*$|#\s*$", re.IGNORECASE)

# 0 은 측정값으로 보고되는 일이 사실상 없고, "0 부터 시작"·"0 건"처럼
# 경계값 서술로 훨씬 자주 쓰인다. 잡으면 오탐만 늘어난다.
...
def _iter_hits(text: str) -> list[str]:
    ...
    for m in RESEARCH_UNIT_RE.finditer(text):
        add("연구 수치", m.group(0))
    # 금액·백분율·맨숫자는 연구 문맥어와 같이 있을 때만 본다.

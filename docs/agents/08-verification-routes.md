...
| **xlsx** | Delivered spreadsheet with live formula errors (`#VALUE! #DIV/0! #REF! #NAME? #NULL! #NUM! #N/A`) | `python skills/xlsx/scripts/recalc.py <file.xlsx>` — or, on Windows without LibreOffice, `python scripts/excel_formula_check.py <file.xlsx>` (drives Excel itself through COM) | **Zero** formula errors reported. Mandatory whenever formulas are used. Note: xlsx has **no** structural/corruption validator — only this formula scan. | §2 |
| **primer-design** (standalone list) | A primer list produced outside the design pipeline (e.g. pasted from a vendor sheet) shipping with a hairpin or homodimer | `python scripts/primer_structure_check.py <SEQ...>` or `--file primers.json` | every primer `hairpin_pass` and `homodimer_pass` true; the stderr summary says N of N passed | §2 |
| **primer-design** (variant screen) | A variant advanced to ordering although one of its QC axes (ddG, primer QC, expression) failed | `python scripts/variant_filter.py --ddg ddg.csv --primer-qc primer.csv [--expression expr.csv] -o matrix.csv` | only rows with `overall_pass == PASS` are ordered; a FAIL row's `notes` names the failing axis | §2, §3 |

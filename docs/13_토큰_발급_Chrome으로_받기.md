...
- **로그인된 계정이 어디 것인지는 Claude가 판단할 수 없습니다.** 화면이 로그인 상태인지는
  ...
  들고 있는 토큰을 발급받게 됩니다 — §3 1단계에서 반드시 먼저 확인하세요.
...
2. Claude가 위 표의 URL로 새 탭을 열고, 로그인된 상태인지 스크린샷으로 확인합니다.
   ...
   - **로그인이 이미 돼 있다면, 화면 우측 상단(또는 계정 메뉴)의 계정명·아바타를 보고
     "이게 내 개인 계정이 맞는지" 반드시 본인이 확인합니다.** 처음 쓰는 PC(집컴, 다른
     사람 노트북 등)일수록 브라우저에 랩 공용/마스터 계정이 이미 로그인돼 있을 위험이
     큽니다 — 그 상태로 토큰을 만들면 해당 PC가 마스터 계정 권한을 그대로 들고 있게
     됩니다. 계정이 다르면 로그아웃 후 본인 계정으로 다시 로그인한 뒤 진행하세요.

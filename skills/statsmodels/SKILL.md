---
name: statsmodels
description: Statistical models library for Python. Use when you need specific model classes (OLS, GLM, mixed models, ARIMA) with detailed diagnostics, residuals, and inference. Best for econometrics, time series, rigorous inference with coefficient tables. For guided statistical test selection with APA reporting use stats-workflow.
...
statsmodels covers five modeling families. The Quick Start above shows the
minimal call for each; the detailed API, options, diagnostics, and worked
examples live in `references/`:

| Family | Use for | Reference |
|--------|---------|-----------|
| Linear regression (OLS/WLS/GLS) | continuous outcomes, inference with coefficient tables | [linear_models.md](references/linear_models.md) |
| Generalized linear models (GLM) | Poisson/logistic/gamma, non-normal outcomes | [glm.md](references/glm.md) |

...
description: Conduct comprehensive, systematic literature reviews using multiple academic databases (PubMed, arXiv, bioRxiv, Semantic Scholar, etc.), producing professionally formatted documents with verified citations (APA, Nature, Vancouver). Use when you need a full structured review document as output. For quick paper lookups or targeted searches use research-search; for brainstorming/discussing findings use research-ideation; for incorporating references into a manuscript use manuscript-pipeline.
...
This skill integrates with multiple scientific skills for database access (research-search, pubmed-database, openalex-database, direct REST APIs) and provides specialized tools for citation verification, result aggregation, and document generation.
...
**⚠️ MANDATORY: Every literature review MUST include at least 1-2 AI-generated figures using the publication-figures skill.**
...
**How to generate schematics:** describe the desired diagram to the **publication-figures** skill (see above) — it handles the generation script and output path internally.
...
1. **Multi-Database Search**:
   ...
   **Specialized Databases:**
   - Use direct REST API access (AlphaFold DB) for protein structures
   - Use direct REST API access (COSMIC) for cancer genomics
   - Use direct REST API access or `research-search` for demographic/statistical data
...
Access via `pubmed-database` skill (direct E-utilities REST API — `esearch.fcgi` / `efetch.fcgi`), or `research-search`, which auto-routes:
```
# Search PubMed via E-utilities esearch
esearch.fcgi?db=pubmed&term=CRISPR+gene+editing&retmax=100

# Search with filters
# Use PubMed Advanced Search Builder to construct complex queries
# Then execute via the pubmed-database skill's E-utilities calls or Bio.Entrez
```
...
Use appropriate skills/access methods:
- **ChEMBL**: direct REST API for chemical bioactivity
- **UniProt**: direct REST API for protein information
- **KEGG**: direct REST API for pathways and genes
- **COSMIC**: direct REST API for cancer mutations
- **AlphaFold**: direct REST API (AlphaFold DB) for protein structures
- **PDB**: direct REST API for experimental structures
...
# - Use pubmed-database skill (or research-search) for PubMed, bioRxiv

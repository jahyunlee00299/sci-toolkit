...
- **Access**: Use `pubmed-database` skill (E-utilities API) or WebFetch tool
...
#### bioRxiv / medRxiv
- **Access**: Use `research-search` skill or direct API
...
# 1. Search PubMed via pubmed-database skill
search_term = "CRISPR AND sickle cell disease"
# Use the pubmed-database skill's E-utilities esearch/efetch calls with search_term

# 2. Search bioRxiv
# Use the research-search skill (or direct bioRxiv API) with search_term

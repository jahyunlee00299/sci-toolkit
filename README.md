...
| `tests/` | 회귀 테스트 25종 (시크릿·연구마커, 참조 실존, 라우팅 정합, 비파괴 설치, 능력 소실, 훅 양방향, 훅 파일 배선, Codex 훅 어댑터, 설치 후 doctor 자동실행, 피드백 채널·정화 게이트, credentials 이원화 감지, 커넥터 dry-run·`--write` 게이트, 서비스·스킬 라우팅 실체 확인, 채택 스킬 조항 실존). `doctor.py` 가 전부 자동 실행 |
